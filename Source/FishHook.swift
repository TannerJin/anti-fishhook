...
public class FishHook {
    @inline(__always)
    public static func replaceSymbol(_ symbol: String,
        ...
        for imgIndex in 0..<_dyld_image_count() {
            if let image = _dyld_get_image_header(imgIndex) {
                replaceSymbolAtImage(image, imageSlide: _dyld_get_image_vmaddr_slide(imgIndex), symbol: symbol, newMethod: newMethod, oldMethod: &oldMethod)
            }
        }
    }
